...
class UserRecord(Base):
    ...
    major: Mapped[str] = mapped_column(String(255), nullable=False, index=True, default="Computer Science")
    modules_taken: Mapped[list[str]] = mapped_column(JsonList, default=list, nullable=False)
    ccas: Mapped[list[str]] = mapped_column(JsonList, default=list, nullable=False)
    nus_opportunities: Mapped[list[str]] = mapped_column(JsonList, default=list, nullable=False)
    exchange_universities: Mapped[list[str]] = mapped_column(JsonList, default=list, nullable=False)
    accommodation: Mapped[str] = mapped_column(String(255), nullable=False, default="Off-campus accommodation")
    ...
    modules_taught: Mapped[list[str]] = mapped_column(JsonList, default=list, nullable=False)
    areas_of_expertise: Mapped[list[str]] = mapped_column(JsonList, default=list, nullable=False)
...
class QuestionRecord(Base):
    ...
    attachments: Mapped[list[str]] = mapped_column(JsonList, default=list, nullable=False)
    key_terms: Mapped[list[str]] = mapped_column(JsonList, default=list, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False, index=True)

    answers: Mapped[list[AnswerRecord]] = relationship(back_populates="question", cascade="all, delete-orphan", order_by="AnswerRecord.created_at")
...
class AnswerRecord(Base):
    ...
    mentor_id: Mapped[str] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
...
class ConversationRecord(Base):
    ...
    messages: Mapped[list[MessageRecord]] = relationship(back_populates="conversation", cascade="all, delete-orphan", order_by="MessageRecord.created_at")
...
class ProfileEmbeddingRecord(Base):
    ...
    embedding: Mapped[list[float]] = mapped_column(JsonList, default=list, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, onupdate=utc_now, nullable=False, index=True)
...
Index("ix_conversations_student_updated", ConversationRecord.student_id, ConversationRecord.updated_at)
